...
const {
  ...
  fetchMore,
} = useCustomQuery(fetchSingleProduct, { product_id: productId.value });
...
onAddProductImage((result) => {
  fetchMore({ product_id: productId.value } as any);
    ...
    if (value.other_medias) {
      ...
      if (error) {
        ...
        const images = urls.map((url) => {
          return { url: url, product_id: productId.value, type: "image" };
        });
