<template>
  <div>
      ...
      <template #content>
        <div class="w-full flex flex-col items-center py-3">
          <h-text-field
            ...
            label="discount_rate"
            placeholder="10%"
            rules="special_discount"
            type="number" />
          <h-text-field
            name="start_date"
            label="start_date"
            placeholder="12/02/2024"
            rules="required|start_date:end_date"
            type="date" />
          <h-text-field
            name="end_date"
            label="end_date"
            placeholder="12/02/2024"
            rules="required|end_date:start_date"
            type="date" />
          <h-text-area
    ...
    <div class="flex justify-between">
      <p class="py-3 text-primary font-bold text-xl">Special Discounts</p>
      <h-button
        v-if="role === 'supplier'"
        @click="showAddDiscountDialog = true"
        type="button"
        btn-name="add_discount"
        class="w-fit mb-4"
        leading-icon="arcticons:cdiscount"
        :gradient="false"
        rounded-class="rounded-md"
        leading-icon-class="bg-primary"
        btn-class="overflow-hidden bg-darkblue text-light400"
    ...
    <div
      ...
      <div
        ...
        <Icon
          ...
          class="w-6 h-6 text-red/25 hover:text-red absolute top-2 right-2 cursor-pointer" />
...
import addDiscount from "~/apollo/mutation/product/realTime/addDiscount.gql";
...
import deleteSpecialDiscount from "~/apollo/mutation/product/realTime/deleteSpecialDiscount.gql";
...
const {
  ...
  onDone: onAddDiscountMutationResult,
  onError: onAddDiscountMutationError,
} = useCustomMutation(addDiscount);
const {
...
const {
  mutate: deleteSpecialDiscountMutation,
  onDone: onDeleteSpecialDiscountResult,
  onError: onDeleteSpecialDiscountError,
} = useCustomMutation(deleteSpecialDiscount);

onDeleteSpecialDiscountError((error) => {
  console.log(error);
  showNotification(
    false,
    error.message.includes("foreign")
      ? "you cant delete as it is already associated to product"
      : "Special Discount Delete Error"
  );
});

onDeleteSpecialDiscountResult((result) => {
  refetchDiscount();
  showConfirmationModal.value = false;
  discountIdToDelete.value = false;
  showNotification(true, "Special Discount Delete Successful");
});
...
onAddDiscountMutationResult((result) => {
  showNotification(true, "Discount Add Successful");
  fetchMore({ supplier_id: uid.value } as any);
  showAddDiscountDialog.value = false;
});
...
const confirmationAction = (response: any) => {
  showConfirmationModal.value = false;
  if (response.accept) {
    deleteSpecialDiscountMutation({ discount_id: discountIdToDelete.value });
  } else {
    discountIdToDelete.value = null;
  }
};
const discountIdToDelete = ref();
const deleteDiscount = (val) => {
  discountIdToDelete.value = val;
