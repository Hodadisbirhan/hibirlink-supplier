...
function useOnMouseMove(el, options, wait) {
    ...
    const pageX = vue.ref(0);
    const pageY = vue.ref(0);
    let handler = (ev) => {
        mouseX.value = ev.x;
        mouseY.value = ev.y;
...
function useOnScroll(el, options, wait) {
    const isValidElement = (el) => isNumber(el) || isBoolean(el) || !(isElement(el) || vue.isRef(el)) || !el
        ? false
        : true;
    const element = isValidElement(el)
        ? wrap(el)
        : vue.ref((isClient && window) || undefined);
    const scrollableElement = isValidElement(el)
        ? element
        : vue.ref((isClient && window.document.scrollingElement) || undefined);
    const scrollTop = vue.ref((scrollableElement.value && scrollableElement.value.scrollTop) || 0);
    const scrollLeft = vue.ref((scrollableElement.value && scrollableElement.value.scrollLeft) || 0);
    let handler = () => {
    ...
    const watchRemoveTop = vue.watch(scrollTop, scrollTopTo, { immediate: false });
    const watchRemoveLeft = vue.watch(scrollLeft, scrollLeftTo, { immediate: false });
    const remove = () => {
        eventRemove();
        watchRemoveLeft();
        watchRemoveTop();
...
function usePromise(fn, lazyOptions) {
    ...
    const exec = async (...args) => {
        ...
        try {
            ...
            if (vue.toRaw(promise.value) === vue.toRaw(currentPromise)) {
                error.value = er;
                result.value = null;
            }
            // if(throwExp){
...
function useCancellablePromise(fn, lazyOptions) {
    ...
    if (vue.getCurrentInstance() &&
        ...
        vue.onUnmounted(() => _cancel && _cancel());
    }
    return {
        ...use,
        cancel,
...
function useRetry(options, factory) {
    ...
    const nextRetry = vue.ref();
    const retryErrors = vue.ref([]);
    const cancellationToken = { value: false };
    const retryId = { value: 0 };
    const retryCount = vue.computed(() => retryErrors.value.length);
    const context = {
        isRetrying,
        retryCount,
        nextRetry,
...
function getDevtools() {
    const empty = {};
    const devtools = vue.inject(DEVTOOLS_KEY, empty);
    if (devtools === empty) {
...
function useDevtoolsInspector(options, nodeList = []) {
    ...
    const nodes = vue.ref(nodeList);
    const byId = vue.computed(() => {
        if (!nodes.value)
            return new Map();
        const r = vue.toRaw(nodes.value);
        const m = new Map();
        for (const i of r) {
            m.set(i.id, i);
        }
    ...
    if (api) {
        ...
        api.on.getInspectorState((payload) => {
            ...
            if (node) {
                const s = vue.reactive(node.state); // unwrap
...
function useFetch(options, requestInitOptions) {
    ...
    const isCancelled = vue.ref(false);
    const cancelledMessage = vue.ref();
    ...
    const status = vue.computed(() => (use.result.value && use.result.value.status) || null);
    const statusText = vue.computed(() => (use.result.value && use.result.value.statusText) || null);
    ...
    if (unmountCancel && vue.getCurrentInstance()) {
        vue.onUnmounted(() => {
            if (abortController) {
                cancel("unmounted");
            }
...
function useWebSocket(url, protocols) {
    ...
    let ws = null;
    const messageEvent = vue.ref(null);
    const errorEvent = vue.ref(null);
...
function useIntersectionObserver(refEl, refOptions) {
    ...
    const element = wrappedElement && (isElement(wrappedElement.value) || !wrappedElement.value)
        ? wrappedElement
        : undefined;
    const options = vue.computed(() => refOptions
        ? unwrap(refOptions)
        : !element
            ? unwrap(refEl)
            : undefined);
    const elements = vue.ref([]);
    const isIntersecting = vue.computed(() => elements.value.length > 0 && elements.value.every(x => x.isIntersecting));
    const handling = (entries) => {
        elements.value = entries;
    };
    let observer = vue.ref();
    if (supported) {
        vue.watch(options, options => {
            if (observer.value) {
...
function useNetworkInformation() {
    ...
    const downlinkMax = vue.ref(0);
    const effectiveType = vue.ref("none");
    const rtt = vue.ref(0);
    const saveData = vue.ref(false);
    const type = vue.ref("none");
    let handler = NO_OP;
    let remove = NO_OP;
    /* istanbul ignore else  */
    if (connection) {
...
function useOnline() {
    ...
    if (!supported) {
        online = vue.ref(false);
    }
    if (!online) {
        online = vue.ref(navigator.onLine);
...
function usePageVisibility() {
    if (!hidden) {
        hidden = vue.ref(isClient && document.hidden);
    }
    if (!visibility) {
        if (isClient) {
            visibility = vue.ref(document.visibilityState);
...
function useLanguage() {
    if (!language) {
        language = isClient ? vue.ref(navigator.language) : vue.ref("");
    }
    if (!languages) {
        if (isClient) {
            languages = vue.ref(navigator.languages);
            const change = () => {
...
function useBroadcastChannel(name, onBeforeClose) {
    ...
    if (supported) {
        ...
        addListener = (cb, o) => {
            ...
            vue.onUnmounted(() => bc.removeEventListener("message", cb));
        };
        vue.onUnmounted(() => {
...
function useGeolocation(options) {
    ...
    const lazy = vue.ref(options ? options.immediate === false : undefined);
    const error = vue.ref(null);
    const timestamp = vue.ref(null);
    const coords = vue.ref(null);
    const highAccuracy = vue.ref((options && options.enableHighAccuracy) || null);
    ...
    if (supported) {
        ...
        let watchId = 0;
        vue.onMounted(() => vue.watch([highAccuracy, lazy], (a) => {
            clearWatch();
...
function useCssVariables(variables, elementOrOptions, optionsConfig) {
    ...
    const [element, options] = vue.isRef(elementOrOptions) || isElement(elementOrOptions)
    ...
    for (let i = 0; i < defEntries.length; i++) {
        ...
        if (!result[key]) {
            ...
            result[key] = vue.ref((vue.isRef(element) && !element.value) || !supported
                ? null
                : getCssVariableFor(unwrap(element), name));
        }
        if (supported) {
            // keep track of changes
            vue.watch([result[key], wrap(element)], (val) => {
                if (!observing)
                    return;
...
function useWorker(stringUrl, args, options) {
    ...
    const errorEvent = vue.ref();
    const data = vue.ref();
    const terminated = vue.ref(!supported);
    const errored = vue.ref(!supported);
    if (!supported) {
...
function useShare(data) {
    ...
    let shared = vue.ref(false);
    let cancelled = vue.ref(false);
    if (supported) {
        share = data => {
            shared.value = false;
            cancelled.value = false;
...
function useWorkerFunction(fn, options) {
    const supported = isClient && "Worker" in self;
    // reactive
    const dependencies = vue.computed(() => (options && unwrap(options.dependencies)) || []);
    const timeoutRef = vue.computed(() => options && unwrap(options.timeout));
    if (!supported) {
    ...
    const promise = useCancellablePromise((...args) => new Promise((res, rej) => {
        ...
        const watchCancel = args.length === fn.length + 1 &&
            vue.isRef(args[args.length - 1]) &&
            args[args.length - 1].value === false
            ? vue.computed(() => promise.cancelled.value || args[args.length - 1].value)
            : promise.cancelled;
        const removeWatch = watchCancel
            ? vue.watch(watchCancel, () => {
                terminate();
                res(undefined);
            }, { immediate: false })
            : NO_OP;
...
function useMatchMedia(query) {
    ...
    let remove = NO_OP;
    if (supported) {
        mediaQueryList = vue.ref(matchMedia(query));
        matches = vue.ref(mediaQueryList.value.matches);
        const process = (e) => {
        ...
        mediaQueryList = vue.ref({});
        matches = vue.ref(false);
    }
    return {
        supported,
        mediaQueryList,
...
function useSharedRef(name, defaultValue) {
    ...
    const id = Date.now();
    const master = vue.ref(false);
    const mind = vue.ref(0 /* HIVE */);
    const editable = vue.computed(() => mind.value === 1 /* MASTER */ ? master.value : true);
...
function useLockScroll(selectorElements, classOptions) {
    const elements = (isString(selectorElements)
        ? vue.ref([]) // it will be resolved when mounted
        : isArray(vue.unref(selectorElements))
    ...
    const tracked = vue.computed(() => elements.value.map(x => vue.ref(x)));
    const remove = vue.watch([locked, tracked], ([l, els], [oldLocked, oldElements]) => {
    ...
    if (isString(selectorElements)) {
        vue.onMounted(() => (elements.value = Array.from(document.querySelectorAll(selectorElements))));
    }
...
function injectFactory(key, defaultValueFactory) {
    return vue.inject(key, defaultValueFactory, true);
}
...
function intlDateFormatExtractArguments(localesOptions, opts) {
    ...
    return isObject(wrappedOpts.value) || vue.isRef(opts)
        ? [
...
function useIntlDateTimeFormat(localesOptions, opts) {
    ...
    const formatter = vue.computed(() => new Intl.DateTimeFormat(unwrap(locales), unwrap(options)));
    ...
    const format = (value, overrideOpts, overrideLocale) => vue.computed(() => formatString(value, overrideOpts, overrideLocale));
    return {
        format,
        formatString,
        formatter,
...
function useIntlNumberFormat(localesOptions, opts) {
    const [locales, options] = intlDateFormatExtractArguments(localesOptions, opts);
    const formatter = vue.computed(() => new Intl.NumberFormat(unwrap(locales), unwrap(options)));
    const formatString = (value, overrideOpts, overrideLocale) => {
        const f = overrideOpts || overrideLocale
...
function useCurrencyFormat(currencyCodeOptions, localesOptions, opts) {
    ...
    const format = (amount, currency, display, opts, overrideLocale) => {
        return vue.computed(() => formatString(amount, currency, display, opts, overrideLocale));
    };
    return {
        format,
        formatString
...
function setBreakpointTailwindCSS(breakpoints) {
    ...
    vue.provide(BREAKPOINT_TAILWIND_KEY, bp);
    return bp;
}
    ...
    if (supported && storage) {
        if (!store) {
            ...
            store = {
                ...
                getRef(k) {
                    const item = vue.ref();
                    let keyWatch = vue.watch(wrap(k), (k) => {
                        const data = storage.getItem(k);
                        if (!data) {
                    ...
                    let valueWatch = vue.watch(item, debounce((i) => {
                ...
                setItem(k, v) {
                    ...
                    this.save(k, v);
                    // @ts-ignore
                    const stop = vue.watch(reference, debounce((r) => {
                        this.save(k, r);
                    }, ms), {
                        immediate: false,
                        deep: true,
...
function usePagination(options) {
    ...
    const lastPage = vue.computed(() => Math.ceil(total.value / pageSize.value));
...
function useArrayPagination(array, options) {
    ...
    const pagination = usePagination({
        ...
        ...options,
        total: vue.computed(() => arrayRef.value.length)
    });
...
function useFormat(format, args) {
    return vue.computed(() => {
        ...
        const r = vue.isRef(args) ? vue.reactive(args.value) : vue.reactive(args);
        const regEx = /({?{[\w\s]*}?})/g;
...
const buildValidationValue = (r, v, handlers) => {
    ...
    const { $pending, $promise, $invalid, $error, $touch, } = buildValidationFunction(r, $validator, vue.ref($message), handlers);
    return {
        $pending,
        $error,
        $promise,
        ...
        if ($value) {
            ...
            if (isValidation(validation)) {
                const validations = Object.keys(validation)
                    .filter((x) => x[0] !== "$")
                    .map((x) => validation[x]);
                $errors = vue.computed(() => validations
                    .map((x) => x.$error)
                    .map((x) => unwrap(x))
                    .filter((x) => x !== undefined));
                // $anyDirty = computed(() => validations.some(x => !!x));
                $anyInvalid = vue.computed(() => validations.some((x) => {
...
function useValidation(input) {
    ...
    const validationInput = vue.reactive(validation.input);
...
function useI18n(definition) {
    if (definition) {
        ...
        return vue.inject(I18n_ACCESS_SYMBOL);
}
/**
...
function buildI18n(definition) {
    const locales = vue.ref(Object.keys(definition.messages));
    const localeMessages = vue.ref(definition.messages);
    const locale = vue.ref(definition.locale);
    const i18n = vue.ref({});
    let fallback = vue.ref();
    const cache = {};
    ...
    const loadLocale = (locale, messages) => {
        ...
        return (cache[locale] = vue.computed(() => retrieveLocaleValue(messages.value[locale])));
    };
    ...
    const localeChangesCount = vue.ref(0);
    vue.watch(localeMessages, () => localeChangesCount.value++, {
        deep: true,
        immediate: false,
    });
    vue.watch([locale, fallback, localeChangesCount], ([l, fb, _]) => {
...
function setI18n(definition) {
    ...
    vue.provide(I18n_ACCESS_SYMBOL, r);
    return r;
}
...
function useSSRTitle(defaultTitle) {
    const s = Symbol();
    const title = vue.inject(SSR_TITLE_KEY, s);
    ...
    if (title === s) {
        ...
        // istanbul ignore next
        return vue.ref(isString(defaultTitle) ? defaultTitle : "");
    }
    if (title === undefined) {
...
function useTitle(overrideTitle = null) {
    ...
    const title = vue.ref(isString(overrideTitle) ? overrideTitle : document.title);
...
const hydrationPlugin = {
    ...
    install(app) {
        ...
        const h = vue.readonly(hydrating);
        {
            // @ts-ignore
...
function useHydration() {
    {
        const s = Symbol();
        const r = vue.inject(HYDRATION_KEY, s);
        if (r === s) {
            console.warn("[useHydration] no hydration found, did you forget to `app.use(HydrationPlugin)`?");
        }
    }
    return vue.inject(HYDRATION_KEY, vue.computed(() => false));
}

function useValueSync(main, args) {
    const master = wrap(main);
    const list = arguments.length === 2
        ? vue.isRef(arguments[1]) && isArray(arguments[1].value)
            ? arguments[1]
            : vue.ref([wrap(arguments[1])])
        : vue.ref(Array.from(arguments)
            .slice(1)
            .map(x => wrap(x)));
